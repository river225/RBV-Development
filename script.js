...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Initialize value history feature
  await initValueHistory();
...
async function fetchValueHistory() {
  try {
    ...
    console.log("No value history found or error loading:", error);
    VALUE_HISTORY_DATA = [];
  }
}

// Add history icon to item card if it has history
function addHistoryIconToCard(itemName, cardElement) {
  // Check if this item has history
  const itemHistory = VALUE_HISTORY_DATA.filter(entry => 
    entry["Item Name"] === itemName || entry.Name === itemName
  );
  
  if (itemHistory.length === 0) return; // No history, don't add icon
  
  // Find the card-left element (image container)
  const cardLeft = cardElement.querySelector('.card-left');
  if (!cardLeft) return;
  
  // Check if icon already exists
  if (cardLeft.querySelector('.history-icon')) return;
  
  // Create history icon
  const icon = document.createElement('div');
  icon.className = 'history-icon';
  icon.innerHTML = '📊';
  icon.title = 'View value history';
  icon.onclick = (e) => {
    e.stopPropagation();
    showHistoryModal(itemName);
  };
  
  cardLeft.appendChild(icon);
}

// Show history modal with graphs
function showHistoryModal(itemName) {
  ...
  if (itemHistory.length === 0) {
    alert("No history available for this item yet.");
    return;
  }
  ...
  modal.innerHTML = `
    <div class="history-modal-header">
      <h2>${itemName} - Value History</h2>
      <button class="history-modal-close" onclick="closeHistoryModal()">×</button>
    </div>
    <div class="history-modal-content">
      <div class="history-time-filters">
        <button class="history-time-filter" data-days="7">Last 7 Days</button>
        <button class="history-time-filter active" data-days="30">Last 30 Days</button>
        <button class="history-time-filter" data-days="90">Last 90 Days</button>
        <button class="history-time-filter" data-days="all">All Time</button>
      </div>
      
      <div class="history-graph-container">
        <h3 class="history-graph-title">Value Over Time</h3>
      ...
      <div class="history-graph-container">
        <h3 class="history-graph-title">Demand Over Time</h3>
        <div class="history-graph-wrapper">
  ...
  // Render charts with default 30 days
  renderHistoryCharts(itemHistory, 30);
  
  // Add filter button listeners
  const filterButtons = modal.querySelectorAll('.history-time-filter');
  filterButtons.forEach(btn => {
    btn.onclick = () => {
      filterButtons.forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      const days = btn.dataset.days;
      renderHistoryCharts(itemHistory, days === 'all' ? 'all' : parseInt(days));
    };
  });
}
...
function renderHistoryCharts(historyData, days) {
  // Filter data by time range
  let filteredData = historyData;
  if (days !== 'all') {
    const cutoffDate = new Date();
    cutoffDate.setDate(cutoffDate.getDate() - days);
    filteredData = historyData.filter(entry => new Date(entry.Timestamp) >= cutoffDate);
  }
  
  if (filteredData.length === 0) {
    document.getElementById('history-stats').innerHTML = '<div class="history-no-data">No data available for this time range</div>';
    return;
  }
  ...
  const values = filteredData.map(entry => parseFloat(entry["Avg Value"] || entry.Value || 0));
  const demands = filteredData.map(entry => parseFloat(entry.Demand || 0));
  
  // Calculate canvas width based on number of data points (40px per point minimum)
  const minWidth = Math.max(650, filteredData.length * 40);
  
  // Destroy existing charts if they exist
  const valueCanvas = document.getElementById('value-chart');
  const demandCanvas = document.getElementById('demand-chart');
  
  if (valueCanvas.chart) valueCanvas.chart.destroy();
  if (demandCanvas.chart) demandCanvas.chart.destroy();
  
  // Set canvas width for horizontal scrolling
  valueCanvas.style.width = minWidth + 'px';
  demandCanvas.style.width = minWidth + 'px';
  
  // Create value chart
  const valueChart = new Chart(valueCanvas, {
    type: 'line',
    data: {
      labels: labels,
      datasets: [{
        ...
        data: values,
        borderColor: '#81e681',
        backgroundColor: 'rgba(129, 230, 129, 0.1)',
        borderWidth: 3,
        tension: 0.3,
        pointRadius: 5,
        pointHoverRadius: 7,
        pointBackgroundColor: '#81e681',
  ...
  valueCanvas.chart = valueChart;
  
  // Create demand chart
  const demandChart = new Chart(demandCanvas, {
    type: 'line',
    data: {
      labels: labels,
      datasets: [{
  ...
  demandCanvas.chart = demandChart;
  
  // Update stats
  updateHistoryStats(filteredData);
}

// Update history stats summary
function updateHistoryStats(historyData) {
  const firstEntry = historyData[0];
  const lastEntry = historyData[historyData.length - 1];
  
  const firstValue = parseFloat(firstEntry["Avg Value"] || firstEntry.Value || 0);
  const lastValue = parseFloat(lastEntry["Avg Value"] || lastEntry.Value || 0);
  const valueChange = lastValue - firstValue;
  const valuePercent = firstValue > 0 ? ((valueChange / firstValue) * 100).toFixed(1) : 0;
  
  const firstDemand = parseFloat(firstEntry.Demand || 0);
  const lastDemand = parseFloat(lastEntry.Demand || 0);
  const demandChange = lastDemand - firstDemand;
  
  const statsHTML = `
    <div class="history-stat-row">
      <span class="history-stat-label">First Recorded:</span>
      <span class="history-stat-value">$${firstValue.toLocaleString()} (Demand: ${firstDemand})</span>
    </div>
    <div class="history-stat-row">
      <span class="history-stat-label">Latest Value:</span>
      <span class="history-stat-value">$${lastValue.toLocaleString()} (Demand: ${lastDemand})</span>
    </div>
    <div class="history-stat-row">
      <span class="history-stat-label">Value Change:</span>
      <span class="history-stat-value ${valueChange >= 0 ? 'up-arrow' : 'down-arrow negative'}">
        ${valueChange >= 0 ? '+' : ''}$${valueChange.toLocaleString()} (${valueChange >= 0 ? '+' : ''}${valuePercent}%)
      </span>
    </div>
    <div class="history-stat-row">
      <span class="history-stat-label">Demand Change:</span>
      <span class="history-stat-value ${demandChange >= 0 ? 'up-arrow' : 'down-arrow negative'}">
        ${demandChange >= 0 ? '+' : ''}${demandChange.toFixed(1)}
      </span>
    </div>
    <div class="history-stat-row">
      <span class="history-stat-label">Total Entries:</span>
      <span class="history-stat-value">${historyData.length}</span>
    </div>
  `;
...
// Initialize history feature when page loads
// This should be called after items are loaded
async function initValueHistory() {
  await fetchValueHistory();
  
  // Add icons to all existing cards
  document.querySelectorAll('.card').forEach(card => {
    const itemName = card.querySelector('.card-info h3')?.textContent?.trim();
    if (itemName) {
      addHistoryIconToCard(itemName, card);
    }
  });
}

// Hook into existing card creation to add history icons
// Store the original createCard function if it exists
const originalCreateCard = window.createCard;

// Override createCard to add history icon
window.createCard = function(item) {
  const card = originalCreateCard ? originalCreateCard(item) : null;
  
  if (card && item.Name) {
    // Small delay to ensure card is in DOM
    setTimeout(() => {
      addHistoryIconToCard(item.Name, card);
    }, 100);
  }
